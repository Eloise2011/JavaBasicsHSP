...
```sql
SELECT 1 AS chapter_order, 'Exception' AS chapter_name, SUM(minutes) / 60 / 60
FROM bilibili_course.courses
WHERE course_name = '韩顺平-零基础30天学会Java'
  AND page_num >= '0443'
  AND page_num <= '0458'

UNION ALL

SELECT 2 AS chapter_order, '集合' AS chapter_name, SUM(minutes) / 60 / 60
FROM bilibili_course.courses
WHERE course_name = '韩顺平-零基础30天学会Java'
  AND page_num >= '0498'
  AND page_num <= '0552'
...
SELECT 2 AS chapter_order, '范型' AS chapter_name, SUM(minutes) / 60 / 60
FROM bilibili_course.courses
WHERE course_name = '韩顺平-零基础30天学会Java'
  AND page_num >= '0553'
  AND page_num <= '0567'
...
SELECT 3 AS chapter_order, 'IO' AS chapter_name, SUM(minutes) / 60 / 60
FROM bilibili_course.courses
WHERE course_name = '韩顺平-零基础30天学会Java'
  AND page_num >= '0614'
  AND page_num <= '0660'
